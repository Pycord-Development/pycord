...
class DPYStandaloneHTMLBuilder(StandaloneHTMLBuilder):
    ...
    def write_genindex(self) -> None:
        ...
        if self.config.html_split_index:
            self.handle_page(
                "genindex", genindexcontext, "genindex-split.html"
            )
            ...
            for (key, entries), count in zip(genindex, indexcounts):
                ...
                self.handle_page(
                    "genindex-" + key, ctx, "genindex-single.html"
                )
        else:
...
def add_builders(app):
    ...
    try:
        ...
        injected_mro = tuple(
            base
            if base is not StandaloneHTMLBuilder
            else DPYStandaloneHTMLBuilder
            for base in original.mro()[1:]
        )
        new_builder = type(
            original.__name__, injected_mro, {"name": "readthedocs"}
        )

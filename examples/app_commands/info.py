...
async def info(ctx, user: pycord.Member = None):
    user = (
        user or ctx.author
    ...
    e.add_field(
        name="Joined",
        value=pycord.utils.format_dt(round(user.joined_at.timestamp()), "F"),
        inline=False,
    )  # When the user joined the server
    e.add_field(
        name="Created",
        value=pycord.utils.format_dt(round(user.created_at.timestamp()), "F"),
        inline=False,

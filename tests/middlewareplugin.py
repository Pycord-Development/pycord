...
from persistqueue import PDict, SQLiteQueue
...
class PyCordTestMiddleware(CacheResponsesPlugin):
    ...
    def handle_client_request(self, request: HttpParser) -> Optional[HttpParser]:
        ...
        if request_tracked:
            request_id = request.header(b"X-PyCord-Testing-ReqId").decode("utf-8")
        ...
        if request.method == b"CONNECT":
            ...
            if request_tracked:
                if tracking_to_bin:
                    first_request = self.request_tracker.pop(f"{request_id}:connect")
                else:
                    first_request = self.request_tracker.pop(request_id)
                merged = CombinedHttpRequest.from_request_pair(first_request, request)
                if tracking_to_bin:
            ...
            destination = (
                request._url.hostname.decode("utf-8")
                if request._url.hostname
                else request._url.remainder.decode("utf-8")
            )

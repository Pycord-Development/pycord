...
class PyCordBaseTestCase(IsolatedAsyncioTestCase):
    ...
    @contextmanager
    def assertRouteCalled(self,
                          route: Route,
                          allow_through_proxy: bool = True
        ...
        with patch.object(HTTPClient, 'request', wrapped_route_request):
            ...
            with patch.object(ClientSession, '_default_headers', create=True, new_callable=PropertyMock, return_value=CIMultiDict({'X-Allow-Through': str(
                    int(allow_through_proxy)), 'X-PyCord-Testing-ReqId': request_bin})):
        ...
        if not route_visited:
            raise AssertionError(f'Route {route.path!r} was not visited, visited URLs: {all_visited_urls}, route: {route.url.split("://")[-1]}')
        else:
    ...
    async def proxied_request(self,
        ...
        try:
            ...
            async with aiohttp.ClientSession(headers={
                ...
                'X-Allow-Through': str(int(allow_through_proxy))
            } | self._tracking_header(reqid) | headers) as cs:

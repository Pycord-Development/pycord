...
class ThreadOption(Option):
    ...
    def __init__(self, thread_type: ValidThreadType) -> types.NoneType:
        warn_deprecated(
            name="ThreadOption",
            instead="Option with the appropriate channel_types instead.",
            since="2.9",
        )
        type_map = {
...
def _get_parameters(
    ...
    if cog:
        parameters.popitem(last=False)  # remove 'self' parameter
    parameters.popitem(last=False)  # remove 'context' parameter

    return parameters


def _get_options(  # pyright: ignore[reportUnusedFunction]
    func: Callable[..., Any], *, cog: type[Cog] | None = None
) -> dict[str, Option]:
    parameters = _get_parameters(func, cog=cog)
...
def _validate_existing_options(
    ...
    validate_names: Callable[[Option], None],
    validate_descriptions: Callable[[Option], None],
    cog: type[Cog] | None = None,
) -> dict[str, Option]:
    parameters = list(_get_parameters(func, cog=cog).items())
    _options = list(options.values())

    for option in _options:
        validate_names(option)
        validate_descriptions(option)
        try:
            name, param = next(iter(parameters))
        except StopIteration:
            raise ValueError(
                f"Too many options specified. Parameter '{option._param_name}' does not exist in function '{func.__name__}'."
            )

        option._handle_type(param.annotation)
        option._param_name = name
